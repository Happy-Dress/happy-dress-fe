...
describe('CatalogHeaderMobile', () => {
    ...
    it('should render correct layout', async () => {
        const { container } = render(<CatalogHeaderMobile filters={response}/>, { wrapper: BrowserRouter });
        await waitFor(() => {
    ...
    it('should del filters', async () => {
        window.history.pushState({}, 'Test Title', '/catalog?categories=4&models=34');
        render(<CatalogHeaderMobile filters={response}/>, { wrapper: BrowserRouter });
        await waitFor(async () => {
            expect(window.location.search).toBe('?categories=4&models=34');
            await act(() => {
                userEvent.click(screen.getByText('Сбросить фильтры'));
            });

            expect(window.location.search).toBe('?categories=4');
        });
    });
});
